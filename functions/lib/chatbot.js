...
async function generateReceiptDirect(env, name, amount, phone, date, event, payment_method, receipt_no) {
  try {
    const receiptNum = receipt_no || await getNextReceiptNumber(env);
    ...
    if (bgBytes) {
      ...
      if (payment_method && F) {
        const pmLabel = payment_method === 'cash' ? '現金' : (payment_method === 'FPS' ? 'FPS轉數快' : (payment_method === 'PayMe' ? 'PayMe' : (payment_method === 'cheque' ? '支票' : payment_method)));
        page.drawText('付款方式: ' + pmLabel, { x: M, y, font: F, size: 12, color: BLACK });
      }
    }
